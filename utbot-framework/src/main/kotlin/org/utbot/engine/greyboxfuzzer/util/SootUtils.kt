    ...
    while (queue.isNotEmpty()) {
        ...
        val implementers =
            if (curSootClass.isInterface) {
                hierarchy.getDirectImplementersOf(curSootClass).filter { it.interfaces.contains(curSootClass) } + hierarchy.getDirectSubinterfacesOf(curSootClass)
            } else {
...
fun SootClass.toJavaClass(): Class<*> = Class.forName(this.name)
